...
/* Custom filtering function for date range in reports table */
$.fn.dataTable.ext.search.push(
    function(settings, data, dataIndex) {
        var minDateStr = $('#min').val();
        var maxDateStr = $('#max').val();
        
        // Получаем дату из колонки "Дата скачивания" (индекс 1)
        var dateTimeStr = data[1] || ''; 
        
        // Если оба поля пустые - показываем все строки
        if (!minDateStr && !maxDateStr) {
            return true;
        }
        
        // Если дата в таблице пустая, скрываем строку
        if (!dateTimeStr) {
            return false;
        }
        
        // Извлекаем только дату из строки "02.10.2025 в 15:21"
        var dateStr = dateTimeStr.split(' в ')[0]; // получаем "02.10.2025"
        
        // Преобразуем дату из формата d.m.Y в объект Date
        var parts = dateStr.split('.');
        if (parts.length !== 3) {
            return false; // некорректный формат даты
        }
        
        var tableDate = new Date(parts[2], parts[1] - 1, parts[0]); // год, месяц (0-11), день
        
        // Преобразуем минимальную дату из фильтра
        var minDate = null;
        if (minDateStr) {
            var minParts = minDateStr.split('.');
            if (minParts.length === 3) {
                minDate = new Date(minParts[2], minParts[1] - 1, minParts[0]);
                minDate.setHours(0, 0, 0, 0); // Начало дня
            }
        }
        
        // Преобразуем максимальную дату из фильтра
        var maxDate = null;
        if (maxDateStr) {
            var maxParts = maxDateStr.split('.');
            if (maxParts.length === 3) {
                maxDate = new Date(maxParts[2], maxParts[1] - 1, maxParts[0]);
        ...
        if (minDate && maxDate) {
            // Обе даты указаны
            return tableDate >= minDate && tableDate <= maxDate;
        } else if (minDate) {
            // Только минимальная дата указана
            return tableDate >= minDate;
        } else if (maxDate) {
            // Только максимальная дата указана
            return tableDate <= maxDate;
        }
...
"<'dt--bottom-section d-sm-flex justify-content-sm-between text-center'<'dt--pages-count  mb-sm-0 mb-3'i><'dt--pagination'p>>",
        "oLanguage": {
            ...
            "sSearch": '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-search"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>',
            "sSearchPlaceholder": "Поиск отчёта...",
           "sLengthMenu": "Количество :  _MENU_",
        },
        "stripeClasses": [],

    ...
    togglethemeEl.addEventListener('click', function() {
                ...
                iframes.forEach(function(iframe) {
                    try {
                        ...
                        if (currentSrc && currentSrc !== 'about:blank') {
                            ...
                            // Создаем URL объект для удобной работы с параметрами
                            var url;
                            try {
                                url = new URL(currentSrc);
                            } catch (e) {
                                // Если URL не валидный, пропускаем этот iframe
                                return;
                            }
                            
                            // Устанавливаем или обновляем параметр theme
                            url.searchParams.set('_theme', newTheme);
                            ...
                            if (newSrc !== currentSrc) {
                                iframe.src = newSrc;
                                console.log('Theme updated in iframe:', newTheme);
                            }
                        }
                    } catch (e) {
        ...
        if (parseObj.settings.layout.darkMode) {
            ...
            if (!getUpdatedParseObject.settings.layout.darkMode) {
                ...
                // Обновляем тему во всех iframe на странице
